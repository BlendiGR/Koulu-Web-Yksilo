...
mpFavBtn.onclick = () => {
  const id = mpHiddenId.textContent;
  if (!id) return;

  const newFavId = favId === id ? "" : id;
  updateFavourite(newFavId);

  if (newFavId) {
    mpFavBtn.classList.add("active");
  } else {
    mpFavBtn.classList.remove("active");
  }
};
...
async function updateFavourite(newFavId) {
  if (!userData) return;
  ...
  if (mpHiddenId.textContent && mpHiddenId.textContent === favId) {
    mpFavBtn.classList.add("active");
  } else {
    mpFavBtn.classList.remove("active");
  }
...
function createCard(r) {
  ...
  card.className = "card";
  if (r._id === favId) card.classList.add("favorite-card");
...
function renderRestaurantCards(restaurants) {
  ...
  sorted.forEach((r) => container.appendChild(createCard(r)));
}
